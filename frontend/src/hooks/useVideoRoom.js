...
export function useVideoRoom(roomId) {
  ...
  const ensurePeer = useCallback(async ({ reset = false } = {}) => {
    if (reset) {
      cleanupPeer();
    }
  ...
  }, [clearReconnectTimer, handleSignal, roomId, setNextChannelState]);

  useEffect(() => {
    mountedRef.current = true;
    manualDisconnectRef.current = false;

    if (hasSupabaseConfig && roomId) {
      connectChannel();
    }
  ...
  }, [cleanupLocalStream, cleanupPeer, clearReconnectTimer, connectChannel, roomId]);
  ...
  }, [cleanupLocalStream, cleanupPeer, clearReconnectTimer, setNextChannelState]);
